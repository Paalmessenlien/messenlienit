...
{case match=1} {* Radio buttons in a row *}
{section var=option loop=$question.options}
    <div class="element {$option.value}"><label><input class="{$option.value}" id="{$attr_id}_{$option.id}" name="{$attr_name}" type="radio" value="{$option.value}"{section show=$option.toggled|eq(1)} checked="checked"{/section} onchange="synchFormElements( '{$attr_id}_{$option.id}', '{$attr_alternative}', false );">{$option.label}</label></div>
{/section}
{if $question.extra_info.enabled|eq(1)}
    <div class="element {$option.value}"><label><input class="{$option.value}" id="{$attr_id}" name="{$attr_name}" type="radio" value="{$question.extra_info.value|wash(xhtml)}"{section show=$question.extra_info.value_checked|eq(1)} checked="checked"{/section} onchange="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );" />{$question.extra_info.label|wash(xhtml)}</label></div>
{/if}
{/case}
{case match=2} {* Radio buttons in a column *}
{section var=option loop=$question.options}
