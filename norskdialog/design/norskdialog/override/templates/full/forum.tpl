...
<div class="content-view-full">
    ...
    {if $node.object.can_create}
        <form method="post" action={"content/action/"|ezurl}>
            <input class="button forum-new-topic" type="submit" name="NewButton" value="Nytt innlegg" />
            <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
            <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
            <input type="hidden" name="ContentLanguageCode" value="{ezini( 'RegionalSettings', 'Locale', 'site.ini')}" />
            <input class="button forum-keep-me-updated" type="submit" name="ActionAddToNotification" value="Hold meg oppdatert om disse innleggene" />
        ...
        <tr class="{$topic.sequence}">
            ...
            {let last_reply=fetch('content','list',hash( parent_node_id, $topic.node_id,
				...
				{section show=$topic_reply_count|gt( 19 )}
                    <a href={concat( $reply.parent.url_alias, '/(offset)/', sub( $topic_reply_count, mod( $topic_reply_count, 20 ) ) , '#msg', $reply.node_id )|ezurl}>Siste kommentar av:</a>
                {section-else}
                    <a href={concat( $reply.parent.url_alias, '#msg', $reply.node_id )|ezurl}>Siste kommentar av:</a>
                {/section}
